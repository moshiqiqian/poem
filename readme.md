...
双击 `work/index.html` 即可打开页面。
 如遇到跨域错误（CORS），请使用方法 2。

##  后端 API 参考 (API Reference)

| HTTP 方法 | 路径                    | 描述                         |
| --------- | ----------------------- | ---------------------------- |
| `GET`     | `/api/poems`            | 获取所有古诗（简要）         |
| `GET`     | `/api/poem/:id`         | 获取指定 ID 的古诗详情       |
| `GET`     | `/api/comments/:poemID` | 获取对应古诗的评论           |
| `POST`    | `/api/comments`         | 提交新评论或回复             |
| `GET`     | `/api/relationships`    | 获取诗人关系图谱节点与边数据 |